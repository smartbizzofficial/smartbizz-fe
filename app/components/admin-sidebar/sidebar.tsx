            ...
            {MENU_BAR_LIST.map((item, i) => {
              return (
                <Link key={i} className="w-full" href={item.path}>
                  <div
                    className={`select-none flex items-center w-full h-[50px] border-b border-b-[#d8d8d8] ${pathname.includes(item.path) ? null : "hover:bg-gray-300"} transition-colors duration-200 cursor-pointer ${pathname.includes(item.path) ? "bg-gray-400 text-white font-semibold" : null}`}
                  >
